...
import ScorecardDetailView from './components/ScorecardDetailView';
...
export default function App() {
  ...
  // State initialization directly from safeStorage
  const [players, setPlayers] = useState<Player[]>(() => {
    const saved = safeStorage.getItem('cricket_players');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { console.error(e); }
    }
    // On very first load, seed with the defaults so they have templates to play with
    safeStorage.setItem('cricket_players', JSON.stringify(SEED_PLAYERS));
    return SEED_PLAYERS;
  });
  ...
  const [teams, setTeams] = useState<Team[]>(() => {
    ...
    safeStorage.setItem('cricket_teams', JSON.stringify(SEED_TEAMS));
    return SEED_TEAMS;
  });
  ...
  const [matches, setMatches] = useState<Match[]>(() => {
    ...
    safeStorage.setItem('cricket_matches', JSON.stringify(SEED_MATCHES));
    return SEED_MATCHES;
  });
  ...
  useEffect(() => {
    ...
    // Start real-time Firestore subscriptions immediately
    const unsubPlayers = subscribePlayers((updatedPlayers) => {
      if (!active) return;
      setPlayers((prev) => {
        if (JSON.stringify(prev) === JSON.stringify(updatedPlayers)) return prev;
        safeStorage.setItem('cricket_players', JSON.stringify(updatedPlayers));
    ...
    const unsubTeams = subscribeTeams((updatedTeams) => {
      if (!active) return;
      setTeams((prev) => {
        if (JSON.stringify(prev) === JSON.stringify(updatedTeams)) return prev;
        safeStorage.setItem('cricket_teams', JSON.stringify(updatedTeams));
        return updatedTeams;
      });
    });
    ...
    const unsubMatches = subscribeMatches((updatedMatches) => {
      if (!active) return;
      setMatches((prev) => {
        if (JSON.stringify(prev) === JSON.stringify(updatedMatches)) return prev;
        safeStorage.setItem('cricket_matches', JSON.stringify(updatedMatches));
        return updatedMatches;
      });
    });

    // Run seed in the background without blocking subscriptions
  ...
  const handleResetData = async () => {
    try {
      ...
      // Seed fresh records directly to Firestore
      ...
      // Update local states & safeStorage
